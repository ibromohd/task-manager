// Task Tracker - final clean implementation
...
function loadTasks() {
    try { return JSON.parse(localStorage.getItem(STORAGE_KEY) || '[]') } catch (e) { console.error('loadTasks', e); return [] }
}
...
function render() {
    ...
    visible.forEach(t => {
        ...
        if (t.completed) label.className = 'completed'
        ...
        editBtn.addEventListener('click', () => startEdit(idx, label))
...
function addTask(text) {
    ...
    tasks.push({ text: text.trim(), completed: false, id: Date.now() })
    saveTasks()
    render()
}
...
function startEdit(index, labelEl) {
    ...
    function onKey(e) {
        ...
        if (e.key === 'Escape') { labelEl.textContent = previous; labelEl.blur() }
    }
...
function startEdit(index, labelEl) {
    ...
    function onKey(e) {
        if (e.key === 'Enter') {
            ...
            // Task Tracker - clean implementation
